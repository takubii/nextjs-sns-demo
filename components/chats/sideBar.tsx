...
export default function SideBar(props: Props) {
  ...
  const getData = async () => {
    try {
      ...
      const { data: profile, error } = await supabase.from('profiles').select().neq('id', user.id);
    ...
    <div className='flex flex-col w-2/5 border-r-2 overflow-y-auto'>
      {profiles.map((item, index) => (
        <div
          ...
          style={{ backgroundColor: selectedId === item.id ? '#eee' : '#fff' }}
          onClick={(e) => {
            handleClick(e);
            handleStyle(item.id);
          }}
        ...
        >
          <div className='w-1/4 min-w-max pointer-events-none'>
            <Image
              src='/user.png'
              className='object-cover h-auto w-12 rounded-full'
              width={30}
              height={30}
              alt=''
            />
          ...
          <div className='w-3/4 pointer-events-none'>
            <div className='text-lg font-semibold'>
              {item.name}
              <span className='block text-gray-500 font-normal text-xs truncate'>
                ID: {item.id}
              </span>
            </div>
          </div>
